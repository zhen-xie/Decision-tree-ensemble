...
cup98_df = pd.read_csv('input/cup98LRN_mini.txt')

X_cup98 = cup98_df.drop(['ODATEDW'], axis=1)
y_cup98 = cup98_df['ODATEDW']
features = list(X_cup98.columns)
...
for i in range(X.shape[1]):
    ...
    for j in range(X.shape[0]):
        if X[j][i] not in index_arr:
            index_arr.append(X[j][i])
            if flag_convert == 0:
                if str(X[j][i]).isdigit() == False:
                    flag_convert = 1
    if flag_convert == 1:
        for j in range(X.shape[0]):
            X[j][i] = index_arr.index(X[j][i])
...
with open("data_cup98.txt", "w+") as f:
...
with open("model_cup98.txt", "w+") as f:

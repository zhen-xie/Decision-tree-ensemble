...
X_train, y_train_multilabel = datasets.load_svmlight_file("./input/train-exp1.svm", multilabel=True)
X = X_train.todense()
y = np.zeros(len(y_train_multilabel))
for i in range(len(y_train_multilabel)):
    if len(y_train_multilabel[i]) != 0:
        y[i] = y_train_multilabel[i][0]
...
with open("data_mediamill.txt", "w+") as f:
...
with open("model_mediamill.txt", "w+") as f:

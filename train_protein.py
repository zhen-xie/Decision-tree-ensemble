...
X = np.zeros([17766, 357])
y = np.zeros(17766)
for index, line in enumerate(open('./input/protein', 'r').readlines()):
    w = line.split('  ')
    y[index] = float(w[0])
    w.pop(0)
    for w_item in w:
        index_w = w_item.split(':')
        if len(index_w) == 2:
            index_w[1] = index_w[1].replace('\n', '')
            X[index][int(index_w[0])-1] = float(index_w[1])
...
with open("data_protein.txt", "w+") as f:
...
with open("model_protein.txt", "w+") as f:

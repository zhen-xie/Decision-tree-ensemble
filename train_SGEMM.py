...
sgemm_df = pd.read_csv('input/sgemm_product.csv')
sgemm_df = sgemm_df.drop(['Run2 (ms)', 'Run3 (ms)', 'Run4 (ms)'], axis=1)

X_sgemm = sgemm_df.drop(['Run1 (ms)'],axis=1)
y_sgemm = sgemm_df['Run1 (ms)']
features = list(X_sgemm.columns)
...
with open("data_SGEMM.txt", "w+") as f:
...
param = {'max_depth': depth, 'eta': 1, 'silent': 1, 'objective': 'binary:logistic', 'nthread': 4,
...
with open("model_SGEMM.txt", "w+") as f:
